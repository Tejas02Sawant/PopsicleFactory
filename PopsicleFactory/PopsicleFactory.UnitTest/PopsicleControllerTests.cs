...
public class PopsicleControllerTests
{
    ...
    {
        _mockService.Setup(s => s.GetAllPopsicles()).ReturnsAsync([]);
    ...
    public async void Get_WithExistingId_ReturnsOk()
    {
        ...
        var result = await _controller.GetById(popsicle1.Id);
    ...
    [Fact]
    public async void Get_WithNewId_ReturnsNotFound()
    {
    ...
    {
        _mockService.Setup(s => s.Search(null, null, null)).ReturnsAsync([]);
        _mapper.Setup(m => m.Map<IEnumerable<PopsicleReadDto>>(It.IsAny<IEnumerable<Popsicle>>())).Returns(new List<PopsicleReadDto>());
